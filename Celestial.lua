...
THIS IS A SIRIUS REWRITE!
--]]


--[[

Celestial Pre-Hyperion Todo List
...
High Priority
 ...
 - http.request support for Celestial Intelligent HTTP Interception
...
Moderate Priority
 ...
 - Reduce any form of detection of Celestial
 - Automated lowering of graphics on lower FPS, ensure no false positives
 
Potential Future Setting Options
 - Block entire domain or just the specific page in the Celestial Intelligent Flow Interception. Do this on case by case, e.g blocked = {"link.com", true} - true being whether its the domain or not
...
local CelestialValues = {
	CelestialVersion = "1.00",
	CelestialName = "Sirius",
	releaseType = "Stable",
	CelestialFolder = "Celestial",
	...
	executors = {"wave", "solara", "awp", "synapse z"},
	...
	pingProfile = {
		...
		adaptiveBaselineSamples = 30, -- how many samples Celestial takes before deciding on a fixed high ping value
...
local CelestialSettings = {
	{
		name = 'General',
		description = 'The general settings for Celestial, from simple to unique features.',
		color = Color3.new(0.117647, 0.490196, 0.72549),
		minimumLicense = 'Free',
		categorySettings = {
			{
				name = 'Anonymous Client',
				description = 'Randomise your username in real-time in any CoreGui parented interface, including Celestial. You will still appear as your actual name to others in-game. This setting can be performance intensive.',
				settingType = 'Boolean',
				current = false,
			...
			{
				name = 'Chat Spy',
				description = 'This will only work on the legacy Roblox chat system. Celestial will display whispers usually hidden from you in the chat box.',
				settingType = 'Boolean',
				current = true,
			...
			},
			{
				...
				description = 'When active, Celestial will notify you when the next song in your Music queue plays.',
				settingType = 'Boolean',
				current = true,
			...
			},
			{
				...
				description = 'Cancel any kick request involving you sent by the client. This may prompt detection from Adonis or similar anti-cheats. You will need to rejoin and re-run Celestial to toggle.',
				settingType = 'Boolean',
				current = false,
	...
	{
		name = 'Keybinds',
		description = 'Assign keybinds to actions or change keybinds such as the one to open/close Celestial.',
		...
		categorySettings = {
			...
			},
			{
				...
				id = 'noclip',
				callback = function()
					local noclip = CelestialValues.actions[1]
			...
			},
			{
				...
				id = 'flight',
				callback = function()
					local flight = CelestialValues.actions[2]
			...
			},
			{
				...
				id = 'refresh',
				callback = function()
					local refresh = CelestialValues.actions[3]
			...
			},
			{
				...
				id = 'respawn',
				callback = function()
					local respawn = CelestialValues.actions[4]
			...
			},
			{
				...
				id = 'invulnerability',
				callback = function()
					local invulnerability = CelestialValues.actions[5]
			...
			},
			{
				...
				id = 'fling',
				callback = function()
					local fling = CelestialValues.actions[6]
			...
			},
			{
				...
				id = 'esp',
				callback = function()
					local esp = CelestialValues.actions[7]
			...
			},
			{
				...
				id = 'nightandday',
				callback = function()
					local nightandday = CelestialValues.actions[8]
			...
			},
			{
				...
				id = 'globalaudio',
				callback = function()
					local globalaudio = CelestialValues.actions[9]
			...
			},
			{
				...
				id = 'visibility',
				callback = function()
					local visibility = CelestialValues.actions[10]
	...
	{
		name = 'Performance',
		description = 'Tweak and test your performance settings for Roblox in Celestial.',
		color = Color3.new(1, 0.376471, 0.168627),
		minimumLicense = 'Free',
		categorySettings = {
			{
				name = 'Artificial FPS Limit',
				description = 'Celestial will automatically set your FPS to this number when you are tabbed-in to Roblox.',
				settingType = 'Number',
				values = {10, 5000},
				current = 240,
			...
			},
			{
				...
				description = 'Celestial will automatically set your FPS to 60 when you tab-out or unfocus from Roblox.',
			...
			},
			{
				...
				description = 'Celestial will check your average latency in the background and notify you if your current latency significantly goes above your average latency.',
				settingType = 'Boolean',
				current = true,
			...
			},
			{
				...
				description = 'Celestial will check your average FPS in the background and notify you if your current FPS goes below a specific number.',
				settingType = 'Boolean',
				current = true,
	...
	{
		name = 'Detections',
		description = 'Celestial detects and prevents anything malicious or possibly harmful to your wellbeing.',
		...
		categorySettings = {
			...
			},
			{
				...
				description = 'Be notified whenever Celestial detects a player joins your session that could be a game moderator.',
				settingType = 'Boolean', 
				minimumLicense = 'Pro',
				current = true,
...
local randomAdjective = CelestialValues.nameGeneration.adjectives[math.random(1, #CelestialValues.nameGeneration.adjectives)]
local randomNoun = CelestialValues.nameGeneration.nouns[math.random(1, #CelestialValues.nameGeneration.nouns)]
...
-- Initialise Celestial Client Interface
local guiParent = gethui and gethui() or coreGui
local Celestial = guiParent:FindFirstChild("Celestial")
if Celestial then
	Celestial:Destroy()
end

local UI = game:GetObjects('rbxassetid://'..CelestialValues.interfaceAsset)[1]
UI.Name = CelestialValues.CelestialName
UI.Parent = guiParent
UI.Enabled = false
...
scriptsPanel.Interactions.Selection.CustomScripts.Subtitle.Text = "Add your own script prompts to Celestial"
-- Interface Caching
...
-- put this into CelestialValues, like the fps and ping shit
...
if not getMessage then CelestialValues.chatSpy.enabled = false end
...
-- Celestial Functions
local function checkCelestial() return UI.Parent end
local function getPing() return math.clamp(statsService.Network.ServerStatsItem["Data Ping"]:GetValue(), 10, 700) end
local function checkFolder() if isfolder then if not isfolder(CelestialValues.CelestialFolder) then makefolder(CelestialValues.CelestialFolder) end if not isfolder(CelestialValues.CelestialFolder.."/Music") then makefolder(CelestialValues.CelestialFolder.."/Music") writefile(CelestialValues.CelestialFolder.."/Music/readme.txt", "Hey there! Place your MP3 or other audio files in this folder, and have the ability to play them through the Celestial Music UI!") end if not isfolder(CelestialValues.CelestialFolder.."/Assets/Icons") then makefolder(CelestialValues.CelestialFolder.."/Assets/Icons") end if not isfolder(CelestialValues.CelestialFolder.."/Assets") then makefolder(CelestialValues.CelestialFolder.."/Assets") end end end
...
local function fetchFromCDN(path, write, savePath)
	pcall(function()
		...
		local file = game:HttpGet(CelestialValues.cdn..path) or nil
		if not file then return end
		if not write then return file end


		writefile(CelestialValues.CelestialFolder.."/"..savePath, file)
...
local function fetchIcon(iconName)
	pcall(function()
		...
		local pathCDN = CelestialValues.icons..iconName..".png"
		local path = CelestialValues.CelestialFolder.."/Assets/"..iconName..".png"
...
local function createEsp(player)
	if player == localPlayer or not checkCelestial() then 
		return
	end
	...
	highlight.Enabled = CelestialValues.actions[7].enabled
	highlight.Parent = espContainer

	player.CharacterAdded:Connect(function(character)
		if not checkCelestial() then return end
		task.wait()
...
local function checkAction(target)
	...
	for _, action in ipairs(CelestialValues.actions) do
...
local function checkSetting(settingTarget, categoryTarget)
	for _, category in ipairs(CelestialSettings) do
...
local function wipeTransparency(ins, target, checkSelf, tween, duration)
	local transparencyProperties = CelestialValues.transparencyProperties
...
local function blurSignature(value)
	if not value then
		if lighting:FindFirstChild("CelestialBlur") then
			lighting:FindFirstChild("CelestialBlur"):Destroy()
		end
	else
		if not lighting:FindFirstChild("CelestialBlur") then
			local blurLight = Instance.new("DepthOfFieldEffect", lighting)
			blurLight.Name = "CelestialBlur"
...
local function checkLastVersion()
	...
	local lastVersion = isfile and isfile(CelestialValues.CelestialFolder.."/".."version.srs") and readfile(CelestialValues.CelestialFolder.."/".."version.srs") or nil

	if lastVersion then
		if lastVersion ~= CelestialValues.CelestialVersion then queueNotification("Celestial has been updated", "Celestial has been updated to version "..CelestialValues.CelestialVersion..", check our Discord for all new features and changes.", 4400701828)  end
	end

	if writefile then writefile(CelestialValues.CelestialFolder.."/".."version.srs", CelestialValues.CelestialVersion) end
end
...
local function removeReverbs(timing)
	...
	for index, sound in next, soundInstances do
		if sound:FindFirstChild("CelestialAudioProfile") then
			local reverb = sound:FindFirstChild("CelestialAudioProfile")
...
local function playNext()
	...
	local asset = getcustomasset(CelestialValues.CelestialFolder.."/Music/"..musicQueue[1].sound)
...
local function addToQueue(file)
	...
	checkFolder()
	if not isfile(CelestialValues.CelestialFolder.."/Music/"..file) then queueNotification("Unable to locate file", "Please ensure that your audio file is in the Celestial/Music folder and that you are including the file extension (e.g mp3 or ogg).", 4370341699) return end
	...
	getLength.SoundId = getcustomasset(CelestialValues.CelestialFolder.."/Music/"..file)
	getLength.Volume = 0
	getLength:Play()
	task.wait(0.05)
...
local function createReverb(timing)
	for index, sound in next, soundInstances do
		if not sound:FindFirstChild("CelestialAudioProfile") then
			local reverb = Instance.new("EqualizerSoundEffect")

			reverb.Name = "CelestialAudioProfile"
			reverb.Parent = sound
...
local function syncExperienceInformation()
	CelestialValues.currentCreator = creatorId

	if creatorType == Enum.CreatorType.Group then
		CelestialValues.currentGroup = creatorId
		CelestialValues.currentCreator = "group"
	end

	for _, gameFound in pairs(CelestialValues.games) do
		if gameFound.id == placeId and gameFound.enabled then
			...
			local rawFile = CelestialValues.rawTree..gameFound.raw
			CelestialValues.currentGame = gameFound
			...
			gameDetectionPrompt.Layer.Run.MouseButton1Click:Connect(function()
				closeGameDetection()
				queueNotification("Running "..gameFound.name, "Now running Celestial' "..gameFound.name.." script, this may take a moment.", 4400701828)
				runScript(rawFile)
...
local function updateSliderPadding()
	for _, v in pairs(CelestialValues.sliders) do
		v.padding = {
...
local function sortActions()	
	...
	for _, action in ipairs(CelestialValues.actions) do
		...
		newAction.Interact.MouseButton1Click:Connect(function()
			...
			if not success then
				queueNotification("Action Error", "This action ('"..(action.name).."') had an error while running, please report this to the Celestial team at Celestial.menu/discord", 4370336704)
				action.enabled = false
				newAction.Icon.Image = "rbxassetid://"..action.images[2]
				tweenService:Create(newAction, TweenInfo.new(0.4, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0.55}):Play()
				tweenService:Create(newAction.UIStroke, TweenInfo.new(0.4, Enum.EasingStyle.Exponential), {Transparency = 0.4}):Play()
	...
	if localPlayer.Character then
		if not localPlayer.Character:FindFirstChildOfClass('Humanoid').UseJumpPower then
			CelestialValues.sliders[2].name = "jump height"
			CelestialValues.sliders[2].default = 7.2
			CelestialValues.sliders[2].values = {0, 120}
		end
	end


	for _, slider in ipairs(CelestialValues.sliders) do
		...
		newSlider.Interact.MouseButton1Down:Connect(function()
			if debounce or not checkCelestial() then return end
...
local function getAdaptiveHighPingThreshold()
	local adaptiveBaselinePings = CelestialValues.pingProfile.adaptiveBaselinePings

	if #adaptiveBaselinePings == 0 then
		return CelestialValues.pingProfile.adaptiveHighPingThreshold
	end
	...
	return median * CelestialValues.pingProfile.spikeThreshold
end

local function checkHighPing()
	local recentPings = CelestialValues.pingProfile.recentPings
	local adaptiveBaselinePings = CelestialValues.pingProfile.adaptiveBaselinePings

	local currentPing = getPing()
	table.insert(recentPings, currentPing)

	if #recentPings > CelestialValues.pingProfile.maxSamples then
		table.remove(recentPings, 1)
	end

	if #adaptiveBaselinePings < CelestialValues.pingProfile.adaptiveBaselineSamples then
...
local function closePanel(panelName, openingOther)
	...
	if not openingOther then
		...
		tweenService:Create(panel, TweenInfo.new(0.65, Enum.EasingStyle.Quint, Enum.EasingDirection.InOut), {Position = CelestialValues.buttonPositions[panelName]}):Play()
...
local function openPanel(panelName)
	...
	panel.Position = CelestialValues.buttonPositions[panelName]
	...
	if panel.Name == "Character" then -- Character Panel Animation
		...
		for _, sliderV in pairs(sliderInfo) do
			if characterPanel.Interactions.Sliders:FindFirstChild(sliderV[1]) then
				...
				for _, sliderFound in ipairs(CelestialValues.sliders) do
...
local function UpdateHome()
	...
	if not table.find(CelestialValues.executors, string.lower(identifyexecutor())) then
		homeContainer.Interactions.Client.Subtitle.Text = "This executor is not verified as supported."
	else
		homeContainer.Interactions.Client.Subtitle.Text = "This executor is supported by Celestial."
	end
...
local function securityDetection(title, content, link, gradient, actions)
	...
	checkFolder()
	local currentAllowlist = isfile and isfile(CelestialValues.CelestialFolder.."/".."allowedLinks.srs") and readfile(CelestialValues.CelestialFolder.."/".."allowedLinks.srs") or nil
			...
			if action[2] then
				if action[3] then
					...
					if currentAllowlist then
						...
						writefile(CelestialValues.CelestialFolder.."/".."allowedLinks.srs", httpService:JSONEncode(currentAllowlist))
					else
						writefile(CelestialValues.CelestialFolder.."/".."allowedLinks.srs", httpService:JSONEncode({domain}))
					end
				end
				decision = true
			else
...
if Essential or Pro then
	getgenv()[index] = function(data)
		if checkCelestial() and checkSetting("Intelligent HTTP Interception").current then
			local title = "Do you trust this source?"
			local content = "Celestial has prevented data from being sent off-client, would you like to allow data to be sent or retrieved from this source?"
			...
			if url == "http://127.0.0.1:6463/rpc?v=1" then
				...
				if bodyDecoded.cmd == "INVITE_BROWSER" then
					...
					content = "Celestial has prevented your Discord client from automatically joining this Discord server, would you like to continue and join, or block it?"
	...
	getgenv()[indexSetClipboard] = function(data)
		if checkCelestial() and checkSetting("Intelligent Clipboard Interception").current then
			local title = "Would you like to copy this to your clipboard?"
			local content = "Celestial has prevented a script from setting the below text to your clipboard, would you like to allow this, or prevent it from copying?"
...
local function onChatted(player, message)
	local enabled = checkSetting("Chat Spy").current and CelestialValues.chatSpy.enabled
	local chatSpyVisuals = CelestialValues.chatSpy.visual

	if not message or not checkCelestial() then return end
	...
	if enabled and player ~= localPlayer then
		...
		if hidden and enabled then
			chatSpyVisuals.Text = "Celestial Spy - [".. player.Name .."]: "..message2
...
local function saveSettings()
	...
	if isfile and isfile(CelestialValues.CelestialFolder.."/"..CelestialValues.settingsFile) then
		writefile(CelestialValues.CelestialFolder.."/"..CelestialValues.settingsFile, httpService:JSONEncode(CelestialSettings))
	end
end
...
local function assembleSettings()
	if isfile and isfile(CelestialValues.CelestialFolder.."/"..CelestialValues.settingsFile) then
		...
		local success, response = pcall(function()
			currentSettings = httpService:JSONDecode(readfile(CelestialValues.CelestialFolder.."/"..CelestialValues.settingsFile))
		end)

		if success then
			for _, liveCategory in ipairs(CelestialSettings) do
			...
			writefile(CelestialValues.CelestialFolder.."/"..CelestialValues.settingsFile, httpService:JSONEncode(CelestialSettings)) -- Update file with any new settings added
		end
	else
		if writefile then
			checkFolder()
			if not isfile(CelestialValues.CelestialFolder.."/"..CelestialValues.settingsFile) then
				writefile(CelestialValues.CelestialFolder.."/"..CelestialValues.settingsFile, httpService:JSONEncode(CelestialSettings))
			end
		end 
	end

	for _, category in CelestialSettings do
	...
	if checkSetting("Client-Based Anti Kick").current then
		if hookmetamethod then 
			...
			originalIndex = hookmetamethod(game, "__index", function(self, method)
				if self == localPlayer and method:lower() == "kick" and checkSetting("Client-Based Anti Kick").current and checkCelestial() then
					queueNotification("Kick Prevented", "Celestial has prevented you from being kicked by the client.", 4400699701)
			...
			originalNamecall = hookmetamethod(game, "__namecall", function(self, ...)
				if self == localPlayer and getnamecallmethod():lower() == "kick" and checkSetting("Client-Based Anti Kick").current and checkCelestial() then
					queueNotification("Kick Prevented", "Celestial has prevented you from being kicked by the client.", 4400699701)
					return
				end
...
local function boost()
	local success, result = pcall(function()
		loadstring(game:HttpGet('https://raw.githubusercontent.com/CelestialSoftwareLtd/Celestial/refs/heads/request/boost.lua'))()
	end)
...
local function start()
	if CelestialValues.releaseType == "Experimental" then -- Make this more secure.
		if not Pro then localPlayer:Kick("This is an experimental release, you must be Pro to run this. \n\nUpgrade at https://Celestial.menu/") return end
	end
	...
	if not checkSetting("Load Hidden").current then 
		...
		--	local startupPath = CelestialValues.CelestialFolder.."/Assets/startup.wav"
		--	local startupAsset
	...
	if CelestialValues.enableExperienceSync then
...
homeContainer.Interactions.Discord.Interact.MouseButton1Click:Connect(function()
	if setclipboard then 
		originalSetClipboard("https://Celestial.menu/discord")
		queueNotification("Discord Invite Copied", "We've set your clipboard to the Celestial discord invite.", 4335479121)
	else
...
for _, button in ipairs(scriptsPanel.Interactions.Selection:GetChildren()) do
	...
	button.Interact.MouseButton1Click:Connect(function()
		...
		if button.name == "InfiniteYield" then
...
userInputService.InputEnded:Connect(function(input, processed)
	...
	if input.UserInputType == Enum.UserInputType.MouseButton1 then
		for _, slider in pairs(CelestialValues.sliders) do
			slider.active = false

			if characterPanel.Visible and not debounce and slider.object and checkCelestial() then
...
players.PlayerAdded:Connect(function(player)
	...
	if checkSetting("Moderator Detection").current and Pro then
		...
		if CelestialValues.currentCreator == "group" then
			for _, role in pairs(CelestialValues.administratorRoles) do 
				if string.find(string.lower(roleFound), role) then
					promptModerator(player, roleFound)
					queueNotification("Administrator Joined", CelestialValues.currentGroup .." "..roleFound.." ".. player.DisplayName .." has joined your session", 3944670656) -- change to group name
				end
			end
		end
	end
...
runService.RenderStepped:Connect(function(frame)
	...
	table.insert(CelestialValues.frameProfile.fpsQueue, fps)
	CelestialValues.frameProfile.totalFPS += fps

	if #CelestialValues.frameProfile.fpsQueue > CelestialValues.frameProfile.fpsQueueSize then
		CelestialValues.frameProfile.totalFPS -= CelestialValues.frameProfile.fpsQueue[1]
		table.remove(CelestialValues.frameProfile.fpsQueue, 1)
	end
end)
...
runService.Stepped:Connect(function()
	...
	if character then
		-- No Clip
		local noclipEnabled = CelestialValues.actions[1].enabled
		local flingEnabled = CelestialValues.actions[6].enabled
...
runService.Heartbeat:Connect(function()
	...
	if primaryPart then
		...
		-- Fly
		if CelestialValues.actions[2].enabled then
			...
			tweenService:Create(bodyVelocity, tweenInfo, { Velocity = velocity * CelestialValues.sliders[3].value * 45 }):Play()
			bodyVelocity.Parent = primaryPart

			if not CelestialValues.actions[6].enabled then
...
while task.wait(1) do
	...
	if disconnectedRobloxUI and not promptedDisconnected then
		...
		for _, preDisconnectType in ipairs(CelestialValues.disconnectTypes) do
	...
	if Pro then
		...
		if checkHighPing() then
			if CelestialValues.pingProfile.pingNotificationCooldown <= 0 then
				if checkSetting("Adaptive Latency Warning").current then
					...
					CelestialValues.pingProfile.pingNotificationCooldown = 120
				end
			end
		end

		if CelestialValues.pingProfile.pingNotificationCooldown > 0 then
			CelestialValues.pingProfile.pingNotificationCooldown -= 1
		end

		-- Adaptive frame time checks
		if CelestialValues.frameProfile.frameNotificationCooldown <= 0 then
			if #CelestialValues.frameProfile.fpsQueue > 0 then
				local avgFPS = CelestialValues.frameProfile.totalFPS / #CelestialValues.frameProfile.fpsQueue

				if avgFPS < CelestialValues.frameProfile.lowFPSThreshold then
					if checkSetting("Adaptive Performance Warning").current then
						...
						CelestialValues.frameProfile.frameNotificationCooldown = 120	
					end
				end
			end
		end
		...
		if CelestialValues.frameProfile.frameNotificationCooldown > 0 then
			CelestialValues.frameProfile.frameNotificationCooldown -= 1
		end
	end
end